...
def cross_validate(y_all, feature_sets, learn_options=None, TEST=False, train_genes=None, CV=True):
    ...
    if learn_options["cv"] == "stratified":
        assert not "extra_pairs" in learn_options or learn_options['extra pairs'], "can't use extra pairs with stratified CV, need to figure out how to properly account for genes affected by two drugs"
